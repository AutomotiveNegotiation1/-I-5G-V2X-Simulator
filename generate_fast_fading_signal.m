 function [zeta,chi,psi] = generate_fast_fading_signal(t,tx_mode,ff_trace,starting_point,interfering_starting_points)
           ...
           switch tx_mode
               case 1
                   % SISO trace
                   zeta  = ff_trace.traces{1,1}.trace.zeta(:,index_position_mod);
                   chi   = [];
                   psi   = ff_trace.traces{1,1}.trace.psi(:,index_position_mod);
               case 2
                   % TxD trace
                   zeta  = ff_trace.trace.zeta(:,:,index_position_mod);
                   chi   = ff_trace.trace.chi(:,:,index_position_mod);
                   psi   = ff_trace.trace.psi(:,:,index_position_mod);
               case {3,4}
                   % OLSM and CLSM trace
