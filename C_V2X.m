...
eNB_height           = 10;       % eNodeB (macro) Tx height in [meter].According to 3GPP 36.814 Table A.2.1.1-2. The transmitter height
...
load('./winer_array_1x1_2.00GHz.mat');
...
if ICIC ==1
    beta_FR            =0.2;     % Band for FR(reuse 1), PR(reuse 3)
else
    beta_FR =1;
end
SINR_threshold     = 15;       % If UE_SINR<SIRN_threshold, this UE is a PR UE
...
%% eNodeB options
antenna_gain_pattern = 'TS 36.942 3D'; % As defined in TS 36.942. Identical to Berger, but with a 65▲ 3dB lobe       
...
trace_length_s=5;
channel_factory_H0 = channel_gain_wrappers.winnerChannelFactory(bandwidth,winner_params,winner_antenna_array);
H_trace0 = generate_FF_trace(trace_length_s/TTI_length,winner_params,winner_antenna_array);
H_trace1 = generate_FF_trace(trace_length_s/TTI_length,winner_params,winner_antenna_array);
...
%% Apped Traffic Model
    ...
    for u_=1:size(UE_Idx,1)
        UEs(u_).traffic_model = trafficmodel_5G(traffic_models_use,UE_Idx(u_),max(scheduler.feedback_channel_delay,0));
...
for TTI_counter=1:TTI_MAX
    ...
    [ue_bs_Rx]=UE_BS_Rx(ALL_VUEs_layout,ALL_eNBs_layout,gNB_tx_dBm,N_PRBs,N_block,pl_new); 
...
l_=1;
 ...
 elseif (ICIC==1)&&(CoMP==0)
     avg_throuput=fopen('nocomp_avg_throughput_Mbps.txt','at');
     fprintf(avg_throuput,'%f\n',average_throughput);
     fclose(avg_throuput);
 else
     avg_throuput=fopen('w.o_avg_throughput_Mbps.txt','at');
     fprintf(avg_throuput,'%f\n',average_throughput);
     fclose(avg_throuput);
 end
...
%         figure(8);
%  for c_ = 1:length(ALL_VUEs_layout)
%             
%                plot( ALL_VUEs_layout(c_,6), ALL_VUEs_layout(c_,7),'--bs');
%                
%                hold on
%  end
